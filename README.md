# este-contato-nao-esta-mais-disponivel
Uma instalação digital weirdcore inspirada no MSN Messenger. Comunicação como vestígio, memória e ausência.

---
# ESTE CONTATO NÃO ESTÁ MAIS DISPONÍVEL

> *Um mensageiro sobre memória, ausência e sistemas que ainda observam.*

---

# Sobre a obra

“Este contato não está mais disponível” não é um erro técnico.  
É uma constatação existencial.

Esta obra é uma instalação digital interativa inspirada na interface do antigo MSN Messenger (Windows 2000/XP), subvertida pela estética Weirdcore / Liminal Space.

Aqui, a comunicação não visa eficiência.  
Ela existe para evocar.

O sistema responde.  
A interface lembra.  
O silêncio também é uma mensagem.

---

# Conceito

A obra simula um ambiente familiar, porém deslocado no tempo e na função.  
Texturas degradadas, filtros visuais instáveis, mensagens fantasma e sons imperfeitos criam uma sensação de nostalgia desconfortável — uma internet que prometia presença constante, mas deixou apenas vestígios.

O usuário não é exatamente um usuário.  
É um visitante.

Este não é um chat social.  
É um resíduo.

---

# Experiência

- Interface inspirada no MSN Messenger 6.2
- Estética weirdcore / dreamcore
- Mensagens automáticas do “sistema”
- Distorção visual e tipográfica
- Efeito Nudge (trepidação)
- Texto curatorial incorporado à interface

Cada sessão é levemente diferente.

---

# Como executar

1. Clone o repositório:
   ```bash
   git clone https://github.com/SEU_USUARIO/este-contato-nao-esta-mais-disponivel.git
````

2. Abra o arquivo:

   ```bash
   index.html
   ```

Não há dependências.
Não há backend.
O arquivo é a obra.

---

# Contexto artístico

Esta obra dialoga com temas como:

* Liminalidade digital
* Nostalgia tecnológica
* Solidão mediada por interfaces
* Memória, ausência e presença simulada
* Weirdcore como linguagem, não estética superficial

---

# Autor

João Vittor da Silva Souza
Web-designer, programador, escritor e artista digital

> “Transformando visões em realidade digital.”

---

# Estado da obra

Esta obra está concluída.
Qualquer modificação posterior deve ser entendida como derivação, não continuação.

```

---

# 4. ESTRUTURA FINAL DO REPOSITÓRIO

```

este-contato-nao-esta-mais-disponivel/
├─ index.html
└─ README.md

```

Nada mais.  
Sem build.  
Sem framework.  
Sem distração.

---

# 5. STATUS FINAL (DECLARAÇÃO)

- Obra: concluída
- Forma: instalação digital
- Meio: GitHub como espaço expositivo
- Função: memória interativa
- Continuidade: encerrada

A partir do momento em que você fizer o commit,  
a obra deixa de ser desenvolvimento e passa a ser arquivo.


